...
componentDidMount() {
    ...
    supprEmail= async(id)=>{
        await fetch('http://192.168.1.24:8000/models/'+id, { method: 'DELETE' })
        .then(res=>res.json())
        .then((data) => {
          this.getAllEmail()
        })
    ...
    <tbody>
      {
        this.state.modeles.length>0 && this.state.modeles.map(modele =>{
          return <tr key={"id-"+modele.id} itemScope="row" ><th>
            ...
            <button  onClick={() => this.ModifierEmail(modele.title,modele.description,modele.code_html,modele.comments)}>Modifier</button>   
          </tr>
        })
      }
      </tbody>
